...
int8_t ICACHE_FLASH_ATTR gizwitsEventProcess(eventInfo_t *info, uint8_t *data, uint32_t len)
{
    ...
    for(i = 0; i < info->num; i++)
    {
        ...
        {
        case EVENT_RGB_DATA:
			currentDataPoint.valueRGB_DATA= dataPointPtr->valueRGB_DATA;
            GIZWITS_LOG("Evt:EVENT_RGB_DATA %d\n",currentDataPoint.valueRGB_DATA);
            //user handle
            if(currentDataPoint.valueRGB_PWR)
		...
		case EVENT_RGB_PWR :
            ...
            {
            	currentDataPoint.valueRGB_DATA = 0x0;
                //user handle
                setAllPixelColor(currentDataPoint.valueRGB_DATA);
            }
            break;
