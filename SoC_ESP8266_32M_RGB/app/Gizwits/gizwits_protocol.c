...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagRGB_DATA)
    {
        info->event[info->num] = EVENT_RGB_DATA;
        info->num++;
        dataPoints->valueRGB_DATA = gizX2Y(RGB_DATA_RATIO,  RGB_DATA_ADDITION, exchangeWord(issuedData->attrVals.valueRGB_DATA));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueRGB_DATA != cur->valueRGB_DATA)
    {
        GIZWITS_LOG("valueRGB_DATA Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueRGB_DATA = exchangeWord(gizY2X(RGB_DATA_RATIO,  RGB_DATA_ADDITION, dataPoints->valueRGB_DATA)); 
